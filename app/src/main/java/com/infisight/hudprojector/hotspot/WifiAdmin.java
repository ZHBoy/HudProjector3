...
public abstract class WifiAdmin {
	...
	public WifiAdmin(Context context) {
		...
		Log.v(TAG, "getIpAddress = " + getIPAddress());
	}
	...
	public boolean addNetwork(WifiConfiguration wcg) {
		...
		Log.i("SettingFragment", wcgID+"000000000000000");
		return b;
	}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(WifiManager.RSSI_CHANGED_ACTION)) {
				...
				if (isWifiContected(mContext) == WIFI_CONNECTED) {
					SpeechVoiceRecognition svr = SpeechVoiceRecognition.getInstance(mContext);
					svr.startSpeaking("网络连接成功");
					stopTimer();
					onNotifyWifiConnected();
					unRegister();
				} else if (isWifiContected(mContext) == WIFI_CONNECT_FAILED) { //在这里进行判断，如果失败，给予失败的提示。
					SpeechVoiceRecognition svr = SpeechVoiceRecognition.getInstance(mContext);
//					svr.startSpeaking("网络连接失败");
					stopTimer();
//					closeWifi();
					onNotifyWifiConnectFailed();
					unRegister();
				} else if (isWifiContected(mContext) == WIFI_CONNECTING) {//在这里给予判断，如果成功，给予成功的提示，并作相应的操作。
	...
	private synchronized void register() {
		...
		myRegisterReceiver(mBroadcastReceiver, new IntentFilter(
